...
public class Slot extends Applet implements Runnable
	...
	private	final	static	String	sEMailHost = "virtualvegas.com",
									...
									sEMailName1 = "bb3rdprize",
									sEMailName2 = "bb2ndprize",
									sEMailName3 = "bb1stprize",
									sEMailName4 = "bbgrandprize" ;
    ...
    ****************************************************************/
    ...
    public boolean handleEvent(Event event)
        ...
        if (event.id == Event.ACTION_EVENT)
			...
			else if (event.target == buttonRealMoney)
				{
				clickedbuttonRealMoney() ;
...
//		smPost.AddMessageLine("IP: " + InetAddress.getLocalHost().toString()) ;
    ...
    ****************************************************************/
	...
	/****************************************************************
    *                                                               *
    * NAME:	clickedbuttonRealMoney (method)							*
    *                                                               *
    ****************************************************************/

	private void clickedbuttonRealMoney()
		{
		URL	urlWebSite ;
		...
		try
			{
			urlWebSite = new URL("http://www.thesands.com/") ;
			}
		...
		if (urlWebSite != null)
			...
			try
				{
				getAppletContext().showDocument(urlWebSite, "_top") ;
				}
...
//			System.out.println("PlayAgainOption") ;
		...
		add(buttonPlayAgain) ;
		buttonPlayAgain.reshape(50, 200, 340, 50) ;

		buttonRealMoney = new Button("Go Play Slots for Real Money at http://www.thesands.com") ;
		add(buttonRealMoney) ;
		buttonRealMoney.reshape(50, 300, 340, 80) ;

//		labelURL = new Label("http://www.wwts.com", Label.CENTER) ;
//		labelURL.reshape(50, 300, 340, 50) ;
