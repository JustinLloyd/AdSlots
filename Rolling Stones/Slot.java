...
*/
	...
	private	final	static	String	sEMailHost = "stonesmart.com",
									sEMailBase = "stonesmart.com",
									sEMailName1 = "slotwinner1",
									sEMailName2 = "slotwinner2",
									sEMailName3 = "slotwinner3",
									sEMailName4 = "slotwinner4" ;

    private final	static long UPDATERATE = 50 ;					// per frame pause (in milliseconds)

	private	final	static	String	sConcertList[] = {	"Sep 23 Soldier Field Chicago",
														"Sep 27 The Ohio Stadium Columbus",
														"Sep 30 Winnepeg Stadium Winnepeg",
														"Oct 02 Commonwealth Stadium Edmonton",
														"Oct 06 Camp Randall Stadium Madison",
														"Oct 08 Rich Stadium Buffalo",
														"Oct 10 Hippodrome De Quebec Quebec City",
														"Oct 12 Veterans Stadium Philadelphia",
														"Oct 16 Giants Stadium New York",
														"Oct 20 Foxboro Stadium Bostn",
														"Oct 23 Jack Kent Cooke Stadium Washington D.C.",
														"Oct 26 Vanderbilt Stadium Nashville",
														"Oct 28 Owen Field Norman",
														"Oct 30 University of New Mexico Albuquerque",
														"Nov 01 Texas Motor Speedway, Ft. Worth",
														"Nov 07 Sundevil Stadium Phoenix",
														"Nov 09 Dodger Stadium Los Angeles",
														"Nov 14 Oakland Stadium Oakland",
														"Nov 22 MGM Grand Garden Arena Las Vegas",
														"Nov 26 Kingdome Seattle",
														"Nov 28 BC Place Stadium Vancouver",
														"Dec 02 Silverdome Detroit",
														"Dec 05 Homestead Racetrack Miami",
														"Dec 09 Georgia Dome Atlanta",
														"Dec 11 TWA Dome St. Louis",
														"Jan 09 Skydone Toronto",
														"Jan 13 Olympic Stadium Montreal",
														"Jan 17 Aloha Stadium Hawaii",
														"Jan 24 HHH Metrodome Minneapolis",
														"Jan 30 Rose Garden Portland",
														"Feb 03 Qualcomm Stadium San Diego",
														"Feb 14 Autodrome Mexico" } ;
...
*/
		...
		boolean bDone = false ;
		for (int nIndex = 0; nIndex < sImageNames.length && !bDone; nIndex++)
			{
...
//					Runtime.getRuntime.gc() ;
    ...
    ****************************************************************/
	...
	private void Details()
        ...
        editName.reshape(112,161,300,22) ;
        ...
        editEMail.reshape(112,201,300,22) ;

		if (SlotMachine.GetCoins() >= 1000)
			{
			label9 = new Label("Select time and") ;
			add(label9) ;
			label9.reshape(0, 240,130, 22) ;

			label10 = new Label("location of concert") ;
			add(label10) ;
			label10.reshape(0, 260,130, 22) ;

			label11 = new Label("you want to try and") ;
			add(label11) ;
			label11.reshape(0, 280,130, 22) ;

			label12 = new Label("win tickets too.") ;
			add(label12) ;
			label12.reshape(0, 300,130, 22) ;


			choiceConcert = new Choice() ;
			for (int nIndex = 0; nIndex < sConcertList.length; nIndex++)
				choiceConcert.addItem(sConcertList[nIndex]) ;

			add(choiceConcert) ;
			choiceConcert.reshape(137, 270, 270, 1200) ;
			}

        buttonSend=new Button("I wanna win!") ;
        add(buttonSend);
        buttonSend.reshape(50,333,350,40) ;

		buttonNoThanks=new Button("No Thanks! I've got more toys than I know what to do with") ;
		add(buttonNoThanks) ;
		buttonNoThanks.reshape(50, 380, 350, 26) ;
		show() ;
		}
    ...
    ****************************************************************/
	...
	private void clickedbuttonSend()
		...
		if (SlotMachine.GetCoins() >= 1000)
			smPost.SetMessage("Player " + sPlayerName + " (" + sPlayerEMail +") won $" + SlotMachine.GetCoins() + " and chose " + "\"" + choiceConcert.getSelectedItem() + "\"") ;
		else
